...
## 2026-06-25 - Rich Text Editor Toolbar Accessibility
**Learning:** ReactQuill toolbars utilizing custom icon-only `<button>` and `<select>` elements for text formatting (e.g., Bold, Italic, Link, Lists, Color/Background select) lacked `aria-label`s. Complex formatting tools are unusable by screen reader users without proper labeling, as the inherent visual iconography (B, I, list icons) is not conveyed programmatically.
**Action:** When implementing or customizing third-party WYSIWYG or rich text editors, always ensure that all formatting controls, especially those using custom UI components like icon buttons or unlabelled select dropdowns, have descriptive `aria-label`s.
