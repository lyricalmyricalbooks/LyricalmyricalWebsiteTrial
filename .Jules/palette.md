...
## 2024-07-01 - Rich Text Editor Toolbar Accessibility
**Learning:** When using custom HTML toolbars for third-party libraries (like ReactQuill), the custom formatting buttons (bold, italic, lists, etc.) may lack default accessibility labels if they are icon-only.
**Action:** Always verify that every custom toolbar control for third-party rich text editors explicitly includes an `aria-label` so that formatting tools remain perceivable to screen reader users.
