...
## 2024-05-26 - Rich Text Editor Accessibility Improvement
**Learning:** Custom toolbar HTML elements in third-party UI libraries (like ReactQuill) must explicitly receive `aria-label` attributes to ensure screen reader accessibility. By default, these icon-only buttons do not have semantic labels which makes them unusable for content managers using assistive technologies.
**Action:** Always inspect the custom toolbars of third-party rich text editors or complex UI components to verify that nested `<button>` and `<select>` elements have properly configured `aria-label` attributes for each action.
