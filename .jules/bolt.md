...
## 2025-02-12 - Eliminate O(N*M) Array Includes Lookups in Cart Iterations
**Learning:** In `Checkout.tsx`, iterating over `cartItems` and running an `Array.prototype.includes()` check against a nested array (like `discount.selectedCategories` or `discount.selectedProducts`) inside a `.some()` or `.filter()` loop results in O(N*M) time complexity. Since this happens inside `useMemo` blocks or handlers that trigger on state changes (like typing a promo code), the repeated O(N*M) string matching blocks the main thread and introduces severe input lag on larger carts.
**Action:** When evaluating constraint arrays against shopping cart loops, always convert the raw array (`discount.selectedCategories`) into an O(1) `Set` *outside* the loop before passing it down. Then inside the loop, use `Set.has(item.id)` instead of `Array.includes(item.id)` to instantly resolve the lookup.
