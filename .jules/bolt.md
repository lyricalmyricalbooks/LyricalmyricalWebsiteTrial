...
## 2026-06-24 - Memoizing Derived State in Context
**Learning:** In a React Context provider (like `CartContext`), defining derived state computations (e.g. `cartTotal` and `cartCount`) directly in the function body without memoization leads to O(N) recalculations on every render, even when the underlying data (`cart`) hasn't changed. This is especially problematic when unrelated context states (like `isCartOpen`) trigger re-renders. Furthermore, not memoizing the Context's `value` object causes unnecessary re-renders in all consumer components whenever the provider re-renders.
**Action:** Use `useMemo` to cache derived calculations (like totals and counts) that depend on arrays, providing the array as the dependency. Additionally, use `useMemo` for the Context `value` object and `useCallback` for context helper functions to ensure consumer components are not forced to re-render when unrelated state updates.
