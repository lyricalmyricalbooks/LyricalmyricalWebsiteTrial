...
## 2024-11-13 - Eliminate O(N*M) lookups for Discount Validations
**Learning:** In `Checkout.tsx`, the discount logic previously used `.includes()` on arrays like `discount.selectedCategories` or `discount.selectedProducts` within loops traversing `cartItems` (`cart.filter`, `cart.some`, `cart.reduce`). This created an unnecessary O(N*M) lookup structure. Every keystroke during email input was potentially triggering these heavy checks due to dependency re-evaluations, further blocking the main thread.
**Action:** When iterating over arrays (like `cartItems`), always pre-convert targeted constraint arrays (such as `selectedCategories` or `selectedProducts`) into O(1) `Set` objects before the loop. Inside the iteration, utilize `Set.has()` instead of `Array.includes()` to replace O(N*M) complexity with an efficient O(N) evaluation sequence.
