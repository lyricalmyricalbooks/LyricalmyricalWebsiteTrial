...
## 2026-07-08 - Eliminating O(N*M) Array Lookups in Checkout Discount Validation
**Learning:** In `Checkout.tsx`, validating constraints such as `selectedCategories` and `selectedProducts` for discounts by repeatedly invoking `.includes()` within `cartItems.some()`, `cartItems.filter()`, and `cartItems.reduce()` creates nested iterations that degrade to O(N*M) complexity. During frequent re-renders or multiple item iterations, this unnecessary array traversal blocks the main thread.
**Action:** Always pre-convert constraint arrays (e.g. `selectedCategories`, `selectedProducts`) into an O(1) `Set` outside the iterative loops, and use `Set.has()` rather than `Array.includes()` to instantly verify elements.
