...
## 2024-11-15 - Eliminating O(N*M) Array Lookups in Checkout Discount Validation
**Learning:** In `Checkout.tsx`, iterating over large shopping cart arrays and repeatedly calling `.includes()` on constraint arrays (like `selectedCategories` or `selectedProducts`) inside the `.filter()` or `.some()` callbacks creates a severe O(N*M) performance bottleneck, frequently blocking the main thread when typing.
**Action:** Always convert filter constraints into O(1) `Set`s before array iteration, and use `Set.has()` instead of `Array.includes()` inside the loop to ensure O(1) lookup speeds and prevent thread blocking.
