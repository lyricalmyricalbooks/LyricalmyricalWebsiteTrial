...
## 2024-07-09 - O(1) Lookups for Array Constraints inside Iterations
**Learning:** In `Checkout.tsx`, iterating over `cartItems` and validating them against constraint arrays like `discount.selectedCategories` or `discount.selectedProducts` using `.includes()` forces O(N*M) lookup bottlenecks during render and validation cycles.
**Action:** Always convert constraint arrays into O(1) `Set` collections outside the loop, and use `Set.has()` instead of `Array.includes()` inside the loop to eliminate O(N*M) lookups when filtering or validating against constraints.
