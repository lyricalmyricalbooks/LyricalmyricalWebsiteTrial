...
## 2026-07-15 - Memoizing filter and sort arrays to prevent redundant O(N log N) evaluations
**Learning:** In large React components like Admin Dashboard tables, computing heavy derived state (such as filtering and sorting an array) directly inline in the component body forces the main thread to block on every render. This becomes very noticeable when unrelated UI state (like opening a context dropdown menu) triggers a re-render, forcing the whole table to be re-filtered and sorted unnecessarily.
**Action:** In large React components, wrap heavy derived calculations (like array filtering and sorting) in `useMemo` to prevent blocking the main thread and causing lag when unrelated UI state (e.g., opening a dropdown menu) changes.
