...
## 2024-11-14 - Eliminating O(N*M) Array Lookups in Checkout Form Validations
**Learning:** In `Checkout.tsx`, the `validateDiscountRestrictions` function and the `discountAmount` `useMemo` block used `.some()` and `.filter()` alongside array `.includes()` to check if specific categories or product IDs existed in a discount's constrained lists. For larger shopping carts and long constraint lists, this nested looping `cartItems.some(item => selectedCats.includes(cat))` caused an $O(N \times M)$ operation that blocked the main thread.
**Action:** When iterating over a loop (like shopping cart validation) and checking against an inclusion list, always convert the list (e.g. `discount.selectedCategories`) into an $O(1)$ `Set` outside of the loop first. Then, perform the checks using `Set.has()`, reducing the overall time complexity to $O(N)$.
