    ...
    <div className="min-h-screen bg-[#030213] text-white selection:bg-white selection:text-black" style={{ fontFamily: font }}>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 md:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24 items-start">
          ...
          {/* Photo Column */}
          <div className={`space-y-4 ${productContentPosition === "left" ? "lg:order-2" : "lg:order-1"}`}>
            {productImageLayout === "slider" ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {photos.map((photo: any, i: number) => (
                  <div key={i} className="aspect-[3/4] bg-neutral-900 rounded-2xl overflow-hidden relative">
                    <img src={photo.url} alt="" className="w-full h-full object-cover" />
                    {i === 0 && isOutOfStock && (
                      <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
                        <span className="border border-white/40 text-white text-[10px] tracking-[0.4em] px-6 py-2 uppercase">SOLD OUT</span>
                      </div>
                    )}
                  </div>
                ))}
          ...
          {/* Info Column */}
          <div className={`flex flex-col justify-center space-y-8 lg:pt-8 ${productContentPosition === "left" ? "lg:order-1" : "lg:order-2"}`}>
            {/* Category */}
            <div>
