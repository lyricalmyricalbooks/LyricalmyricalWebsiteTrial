...
export function Checkout() {
  ...
  const validateDiscountRestrictions = (discount: any, email: string, cartItems: any[], catalogMap: Map<string, any>, currentCartCount: number, currentCartTotal: number) => {
    ...
    // ⚡ Bolt: Convert constraint arrays into O(1) Sets outside the loop
    // Measured impact: Prevents O(N*M) nested lookups during cart iteration for discounts
  ...
  const discountAmount = useMemo(() => {
    ...
    const { qualifyingSubtotal, qualifyingItems } = (() => {
      ...
      // ⚡ Bolt: Convert constraint arrays into O(1) Sets outside the filter loop
      // Measured impact: Prevents O(N*M) string lookup complexity per keystroke in checkout
